...
| `OVERPASS_URL`                      | Overpass API endpoint used by the core Overpass client (POI fallbacks, navigation alerts, junction-view gantry lookups). The public instance allows two concurrent queries per IP, so multi-user deployments should point this at the self-hosted `overpass` service.                                                                                                            | Optional. Commented `http://overpass:80`         |
