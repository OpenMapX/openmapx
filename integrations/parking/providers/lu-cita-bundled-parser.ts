 ...
 *     de-parkapi-v2-bundled-parser.ts and de-parkapi-v3-bundled-parser.ts;
...
async function fetchStatus(log: PoiSourceLogger): Promise<DatexParkingStatus[]> {
  try {
    ...
    if (!res.ok) {
      log.warn("lu-cita: status feed returned non-2xx", { status: res.status });
      return [];
    }
    const text = await res.text();
    return parseDatexParkingStatus(text);
  } catch (err) {
    log.warn("lu-cita: status feed failed", { error: (err as Error).message });
    return [];
  }
}

export const parseLuCitaBundled: PoiBundledParseFn = async (buffer, { log }) => {
  const tableXml = buffer.toString("utf-8");
  let records: DatexParkingRecord[];
  try {
    records = parseDatexParkingTable(tableXml);
  } catch (err) {
    log.warn("lu-cita: failed to parse parking table", { error: (err as Error).message });
