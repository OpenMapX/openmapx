 ...
 *
 * The companion `de-apag-mobidrom` source ingests the same operator's data via
...
export function parseDeApagBundled(): PoiBundledParseFn {
  return async (buffer, { log }) => {
    let data: unknown;
    try {
      data = JSON.parse(buffer.toString("utf-8"));
    } catch (err) {
      log.warn("de-apag: failed to parse facilities JSON", {
        error: (err as Error).message,
      });
      return { static: [], live: new Map<string, PoiLiveState>() };
    }
    if (!Array.isArray(data)) {
      log.warn("de-apag: facilities payload is not an array", {
        actual: typeof data,
      });
      return { static: [], live: new Map<string, PoiLiveState>() };
    }
    ...
    if (skippedNonParking > 0 || skippedBadCoords > 0) {
      log.info("de-apag: parse summary", {
