 ...
 * Same WHY as `parkapi-v2-bundled-parser.ts`: declaring one PoiSource per
...
async function fetchRoadParking(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      log.debug?.("autobahn-de: road fetch returned non-2xx", { road, status: res.status });
      return [];
    }
    const data = (await res.json()) as { parking_lorry?: AutobahnParkingLorry[] };
    return data.parking_lorry ?? [];
  } catch (err) {
    log.warn("autobahn-de: road fetch failed", { road, error: (err as Error).message });
    return [];
  }
}

export const parseAutobahnDeBundled: PoiBundledParseFn = async (buffer, { log }) => {
  let roads: string[];
  try {
    const parsed = JSON.parse(buffer.toString("utf-8")) as { roads?: string[] };
    roads = parsed.roads ?? [];
  } catch (err) {
    log.warn("autobahn-de: failed to parse roads list", { error: (err as Error).message });
