...
const DE_RP_TRIER_FACILITIES: Record<string, DeRpTrierFacility> = {
...
interface DeRpTrierXmlEnvelope {
  parken?: {
    datum?: unknown;
    uhrzeit?: unknown;
    parkhaus?: DeRpTrierXmlEntry | DeRpTrierXmlEntry[];
  };
}

interface DeRpTrierXmlEntry {
  phname?: unknown;
  phstate?: unknown;
  shortmax?: unknown;
  shortfree?: unknown;
 ...
 */
function buildAsOf(envelope: DeRpTrierXmlEnvelope["parken"]): string {
...
export const parseDeRpTrierBundled: PoiBundledParseFn = (buffer, { log }) => {
  ...
  let envelope: DeRpTrierXmlEnvelope;
  try {
    envelope = parseXmlDocument(text) as DeRpTrierXmlEnvelope;
  } catch (err) {
  ...
  for (const entry of entries) {
    ...
    if (!phname) continue;
    const facility = DE_RP_TRIER_FACILITIES[phname];
    if (!facility) {
