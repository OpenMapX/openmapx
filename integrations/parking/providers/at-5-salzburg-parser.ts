...
interface At5SalzburgFeatureProperties {
  ID?: number;
  BEZEICHNUNG?: string;
  ADRESSE?: string | null;
  TYP?: string;
...
interface At5SalzburgFeature {
  type: "Feature";
  id?: string;
  geometry?: { type: "Point"; coordinates: [number, number] };
  properties?: At5SalzburgFeatureProperties;
}

interface At5SalzburgGeoJsonResponse {
  type?: "FeatureCollection";
  features?: At5SalzburgFeature[];
}
...
export const parseAt5SalzburgBundled: PoiBundledParseFn = (buffer) => {
  let data: At5SalzburgGeoJsonResponse;
  try {
    data = JSON.parse(buffer.toString("utf-8")) as At5SalzburgGeoJsonResponse;
  } catch {
