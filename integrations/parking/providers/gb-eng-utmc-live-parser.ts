 ...
 * JSON array of GbEngUtmcDynamicCarPark records. The PoiLiveState carries the
 * raw occupancy + stateDescription; mergeGbEngUtmcLive derives freeSpaces and
 * the canonical state enum from there + the static capacity.
 */

interface GbEngUtmcDynamicCarPark {
...
export const parseGbEngUtmcLive: PoiLiveParseFn = (buffer) => {
  ...
  for (const record of data as GbEngUtmcDynamicCarPark[]) {
