 ...
 * apag.de PMS API directly. The companion `de-apag-mobidrom` reader is kept as
 * a backup data lineage going through the NRW Mobilithek exporter. The
 * Mobidrom-shaped payload (see de-apag-parser.ts) lets us reuse the Mobidrom
 ...
 * `de-apag-mobidrom` source.
 */

const STATION_ID_PREFIX = "de-apag:";
...
export async function searchDeApag(bbox: BoundingBox): Promise<ParkingFacility[]> {
  return reader.search(getRuntimeContext(), toBboxTuple(bbox));
}

export async function fetchDeApagDetail(externalId: string): Promise<ParkingFacility | null> {
