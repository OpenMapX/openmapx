...
import { parseAutobahnDeBundled } from "../autobahn-de-bundled-parser.js";
 ...
 * Source id (`autobahn-de`) and prefix (`autobahn:`) unchanged. Static-only:
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const { static: rows } = await parseAutobahnDeBundled(ROADS_FIXTURE, { log: noopLog });
  return rows.map((row) => mapAutobahnDePayload(row.poiId, row.payload));
}
...
describe("autobahn-de parser+mapper equivalence to pre-migration in-memory impl", () => {
  ...
  it("dedupes identifiers that appear on multiple roads", async () => {
    ...
    expect(ids.filter((id) => id === "autobahn:A1-RAST-001")).toHaveLength(1);
  });

  it("filters out future-only and bad-coordinate entries", async () => {
    const got = await runMigrated();
    const ids = got.map((f) => f.id);
    expect(ids).not.toContain("autobahn:A1-FUTURE");
  });
});
