...
describe("parseEsMdMadridStatic", () => {
  it("skips entries without location and yields the rest", () => {
    const rows = parseEsMdMadridStatic(FIXTURE);
  ...
  it("returns [] for non-JSON or missing @graph", () => {
    expect(parseEsMdMadridStatic(Buffer.from("not json"))).toEqual([]);
    expect(parseEsMdMadridStatic(Buffer.from(JSON.stringify({})))).toEqual([]);
  });
});
