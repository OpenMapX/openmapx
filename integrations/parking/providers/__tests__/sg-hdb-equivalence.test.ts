...
import { parseSgHdbLive } from "../sg-hdb-live-parser.js";
import { mapSgHdbPayload, mergeSgHdbLive } from "../sg-hdb-mapper.js";
import { parseSgHdbStatic, svy21ToWgs84 } from "../sg-hdb-static-parser.js";
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const rows = await parseSgHdbStatic(STATIC, { log: noopLog });
  const liveMap = await parseSgHdbLive(LIVE, { log: noopLog });
  const out: ParkingFacility[] = [];
  for (const row of rows as { poiId: string; lng: number; lat: number; payload: unknown }[]) {
    const base = mapSgHdbPayload(row.poiId, row.payload);
    const merged = mergeSgHdbLive(base, liveMap.get(row.poiId) ?? null);
