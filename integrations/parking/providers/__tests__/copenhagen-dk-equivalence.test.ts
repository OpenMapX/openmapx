...
import { parseCopenhagenDkStatic } from "../copenhagen-dk-parser.js";

/**
 * Pre-migration reference, lifted verbatim from the prior copenhagen-dk.ts.
 * Source id is unchanged ("copenhagen-dk", "copenhagen:" prefix).
 */
...
function runMigrated(): ParkingFacility[] {
  return parseCopenhagenDkStatic(FIXTURE).map((row) =>
    mapCopenhagenPayload(row.poiId, row.payload),
  );
}
