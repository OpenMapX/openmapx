...
import { parseRdwNlStatic } from "../rdw-nl-parser.js";

/**
 * Pre-migration reference: query 3 GEO datasets + specs, join by
 * (areamanagerid, areaid). Source id is unchanged ("rdw-nl", "rdw:" prefix).
 */
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const iter = parseRdwNlStatic(SPECS, { log: noopLog });
  const list: ParkingFacility[] = [];
  for await (const row of iter) {
    list.push(mapRdwNlPayload(row.poiId, row.payload));
  }
  return list;
}
