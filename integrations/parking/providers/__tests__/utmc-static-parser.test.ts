...
describe("parseUtmcStatic", () => {
  ...
  it("falls back to 'Car Park <id>' name when shortDescription is missing", () => {
    ...
    const rows = parseUtmcStatic(buffer);
    expect(rows[0]?.payload.name).toBe("Car Park CPX");
  });

  it("returns [] for non-JSON or non-array input", () => {
    expect(parseUtmcStatic(Buffer.from("not json"))).toEqual([]);
    expect(parseUtmcStatic(Buffer.from(JSON.stringify({ not: "array" })))).toEqual([]);
  });
});
