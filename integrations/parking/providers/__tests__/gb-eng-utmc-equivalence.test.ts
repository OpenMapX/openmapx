 ...
 * the current `gb-eng-utmc` naming.
 */

interface GbEngUtmcStaticCarParkRef {
...
interface GbEngUtmcDynamicCarParkRef {
  systemCodeNumber: string;
  dynamics: Array<{
    occupancy?: number;
...
function refStaticToFacility(
  record: GbEngUtmcStaticCarParkRef,
  dynamic: GbEngUtmcDynamicCarParkRef | undefined,
  now: number,
  ...
  return {
    id: `gb-eng-utmc:${record.systemCodeNumber}`,
...
function runReference(): ParkingFacility[] {
  const now = Date.now();
  const statics = JSON.parse(STATIC.toString("utf-8")) as GbEngUtmcStaticCarParkRef[];
  const dyns = JSON.parse(DYNAMIC.toString("utf-8")) as GbEngUtmcDynamicCarParkRef[];
  const dynMap = new Map<string, GbEngUtmcDynamicCarParkRef>();
  for (const d of dyns) {
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const rows = parseGbEngUtmcStatic(STATIC);
  const liveMap = await parseGbEngUtmcLive(DYNAMIC, { log: noopLog });
  return rows.map((row) => {
    const base = mapGbEngUtmcPayload(row.poiId, row.payload);
    const live = liveMap.get(row.poiId) ?? null;
    return mergeGbEngUtmcLive(base, live);
  });
}
