 ...
 * Source id is unchanged ("brussels-be", "brussels:" prefix), so equivalence
 * is exact across the migration.
 */
...
function runMigrated(): ParkingFacility[] {
  return parseBrusselsBeStatic(FIXTURE).map((row) => mapBrusselsPayload(row.poiId, row.payload));
}
