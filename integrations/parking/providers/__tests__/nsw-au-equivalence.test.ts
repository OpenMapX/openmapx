...
import { parseNswAuBundled } from "../nsw-au-bundled-parser.js";
 ...
 * per-facility detail response. Source id is unchanged ("nsw-au", "nsw:"
 * prefix).
 */
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const { static: rows, live } = await parseNswAuBundled(LIST, { log: noopLog });
  return rows.map((row) => {
    const base = mapNswAuPayload(row.poiId, row.payload);
    return mergeNswAuLive(base, live.get(row.poiId) ?? null);
  });
}
