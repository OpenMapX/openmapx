 ...
 * Source id is `ch-bs-basel` (prefix `ch-bs-basel:`).
 */
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const { static: rows, live } = await parseChBsBaselBundled(FIXTURE, { log: noopLog });
  return rows.map((row) => {
    const base = mapChBsBaselPayload(row.poiId, row.payload);
    const merged = mergeChBsBaselLive(base, live.get(row.poiId) ?? null);
