...
import { makeParkApiV3BundledParser } from "../parkapi-v3-bundled-parser.js";
...
async function runMigrated(): Promise<ParkingFacility[]> {
  const parse = makeParkApiV3BundledParser();
  const { static: rows, live } = await parse(FIXTURE_BUFFER, { log: noopLog });
  return rows.map((row) => {
    const base = mapParkApiV3Payload(row.poiId, row.payload);
    return mergeParkApiV3Live(base, live.get(row.poiId) ?? null);
  });
}
...
describe("parkapi-v3 parser+mapper equivalence to pre-migration impl", () => {
  ...
  it("skips bike/missing-coord sites just like the pre-migration impl", async () => {
    ...
    expect(ids).not.toContain("parkapi-v3:2");
    expect(ids).not.toContain("parkapi-v3:5");
    expect(ids).toEqual(["parkapi-v3:1", "parkapi-v3:3", "parkapi-v3:4"]);
  });
});
