...
describe("parseUtmcLive", () => {
  it("returns a Map keyed by systemCodeNumber", async () => {
    const out = await parseUtmcLive(FIXTURE, { log: noopLog });
  ...
  it("falls back to now() when upstream omits lastUpdated", async () => {
    ...
    const out = await parseUtmcLive(buffer, { log: noopLog });
  ...
  it("returns empty map for non-JSON or non-array input", async () => {
    expect((await parseUtmcLive(Buffer.from("not json"), { log: noopLog })).size).toBe(0);
    expect(
      (await parseUtmcLive(Buffer.from(JSON.stringify({ not: "array" })), { log: noopLog })).size,
    ).toBe(0);
  });
});
