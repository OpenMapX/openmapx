 ...
 * JSON array of UtmcStaticCarPark records. We yield one PoiRow per record
 ...
 * `realtimeDataUpdatedAt`) — those land via the live parser + mergeUtmcLive.
 */
...
export function parseUtmcStatic(buffer: Buffer): PoiRow[] {
  ...
  const out: PoiRow[] = [];
  for (const record of data as UtmcStaticCarPark[]) {
