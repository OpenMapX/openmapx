...
export const parseDeByBambergBundled: PoiBundledParseFn = (buffer, { log }) => {
  let data: BambergResponse;
  try {
    data = JSON.parse(buffer.toString("utf-8")) as BambergResponse;
  } catch (err) {
    log.warn("de-by-bamberg: failed to parse JSON", { error: (err as Error).message });
  ...
  for (const entry of data.carParks) {
    ...
    if (!facility) {
      log.warn("de-by-bamberg: unknown facility id, skipping", { id: poiId, name: entry.name });
      continue;
    }
