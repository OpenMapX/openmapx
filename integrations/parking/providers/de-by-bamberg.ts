...
import { mapDeByBambergPayload, mergeDeByBambergLive } from "./de-by-bamberg-mapper.js";
...
const STATION_ID_PREFIX = "de-by-bamberg:";

const reader = createTwoTierPoiReader<ParkingFacility>({
  sourceId: "de-by-bamberg",
  mapStatic: mapDeByBambergPayload,
  mergeWithLive: mergeDeByBambergLive,
...
export async function searchDeByBamberg(bbox: BoundingBox): Promise<ParkingFacility[]> {
  return reader.search(getRuntimeContext(), toBboxTuple(bbox));
}

export async function fetchDeByBambergDetail(id: string): Promise<ParkingFacility | null> {
