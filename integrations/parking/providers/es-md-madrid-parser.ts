...
function parseOpeningHours(org: EsMdMadridOrganization | undefined): string | undefined {
...
function formatAddress(addr: EsMdMadridAddress | undefined): string | undefined {
...
export function parseEsMdMadridStatic(buffer: Buffer): PoiRow[] {
  ...
  let data: EsMdMadridApiResponse;
  try {
    data = JSON.parse(sanitized) as EsMdMadridApiResponse;
  } catch {
    return [];
  }
