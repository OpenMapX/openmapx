...
import { parseBnetzaCsv } from "../bnetza-parser.js";
...
// poi-ingest migration. The ONLY field that's expected to differ is
// `sources`: the registry id renamed from "bnetza" to "bnetza-ev" per
// locked decision #11. Everything else — id, sourceItemIds, name, coordinates,
...
function runMigrationParse(buffer: Buffer): EvChargingStation[] {
  ...
  for (const row of parseBnetzaCsv(buffer)) {
    out.push(mapBnetzaPayload(row.poiId, row.payload));
  }
  return out;
}
...
describe("bnetza parser+mapper equivalence to pre-migration in-memory parser", () => {
  it("produces stations whose only difference is the namespaced source id", () => {
    ...
    // Re-tag the reference with the post-migration source id so the rest of
    // the comparison is field-by-field.
    const normalized = reference.map<EvChargingStation>((s) => ({
      ...s,
      sources: ["bnetza-ev"],
    }));
