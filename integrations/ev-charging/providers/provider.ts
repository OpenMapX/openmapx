...
export class EvChargingProvider implements MobilityDataSourceProvider {
  ...
  constructor(private readonly sources = EV_CHARGING_SOURCE_REGISTRY) {}
  ...
  private async gatherMergedStations(
    bbox: BoundingBox,
    filters?: Record<string, unknown>,
  ): Promise<EvChargingStation[]> {
    const results = await Promise.allSettled(
      this.sources.map((source) => source.search(bbox, filters)),
    );
    ...
    const allStations = results.flatMap((result, i) => {
      ...
      rejectedCount++;
      this.log?.warn(`ev-charging source ${this.sources[i].id} failed`, result.reason);
      return [];
    });
    if (rejectedCount > 0 && rejectedCount === results.length) {
      this.log?.error("all ev-charging sources failed");
    }
    return deduplicateChargingStations(allStations);
  }
  ...
  async search(
    ...
    const merged = await this.gatherMergedStations(bbox, filters);
  ...
  async searchStations(bbox: BoundingBox): Promise<EvChargingStation[]> {
    return this.gatherMergedStations(bbox);
  }
  ...
  private async enrichStation(station: EvChargingStation): Promise<EvChargingStation> {
    ...
    const results = await Promise.allSettled(this.sources.map((source) => source.search(bbox)));
