...
export function* parseBnetzaCsv(buffer: Buffer): Iterable<PoiRow> {
...
export async function resolveBnetzaCsvUrl(log: PoiSourceLogger): Promise<string> {
  try {
    ...
    if (url) return url;
    log.warn("bnetza-parser: scraped page contained no CSV URL — using fallback");
  } catch (err) {
    log.warn(`bnetza-parser: scrape failed (${(err as Error).message}) — using fallback`);
  }
