...
// switzerland-parser.ts treats its buffer.
export const DOTNL_LOCATIONS_URL = "https://opendata.ndw.nu/charging_point_locations_ocpi.json.gz";
...
async function fetchTariffMap(log: PoiSourceLogger): Promise<Map<string, EvChargingTariff>> {
  try {
    const res = await globalThis.fetch(DOTNL_TARIFFS_URL, {
      signal: AbortSignal.timeout(TARIFFS_FETCH_TIMEOUT_MS),
    });
    if (!res.ok) {
      log.error(
        `netherlands-parser: DOT-NL tariffs unavailable this run — stations will have no pricing until next successful fetch: HTTP ${res.status}`,
      );
      return new Map();
    }
    ...
    log.error(
      `netherlands-parser: DOT-NL tariffs unavailable this run — stations will have no pricing until next successful fetch: ${(err as Error).message}`,
    );
    return new Map();
  }
}

async function* dotNlAsyncIterable(buffer: Buffer, log: PoiSourceLogger): AsyncIterable<PoiRow> {
...
export const parseDotNl: PoiStaticParseFn = (buffer, { log }) => dotNlAsyncIterable(buffer, log);
