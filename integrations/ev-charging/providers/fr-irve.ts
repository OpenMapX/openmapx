...
interface FrIrveResponse {
  total_count?: number;
  results?: FrIrveRecord[];
}
...
function recordId(record: FrIrveRecord): string | undefined {
  return (
...
function recordCoordinates(record: FrIrveRecord): [number, number] | null {
  const lng =
...
function paymentMethods(record: FrIrveRecord): string[] | undefined {
...
function recordConnectors(record: FrIrveRecord): EvChargingConnector[] {
...
function recordAttribution(record: FrIrveRecord): EvChargingStation["attributions"] {
...
function recordToStation(record: FrIrveRecord): EvChargingStation | null {
...
async function fetchRecords(params: URLSearchParams): Promise<FrIrveResponse> {
  return fetchJson<FrIrveResponse>(`${RECORDS_URL}?${params.toString()}`, {
    errorMessage: ({ status }) => `France IRVE API error: ${status}`,
  });
}

export async function searchFrIrveCharging(bbox: BoundingBox): Promise<EvChargingStation[]> {
...
export async function fetchFrIrveChargingDetail(itemId: string): Promise<EvChargingStation | null> {
  const id = itemId.startsWith("fr-irve:") ? itemId.slice("fr-irve:".length) : itemId;
...
export const frIrveSource: EvChargingSource = {
  id: "fr-irve",
  priority: getEvChargingSourcePriority("fr-irve"),
  search: searchFrIrveCharging,
  canFetchDetail: (itemId) => itemId.startsWith("fr-irve:"),
  fetchDetail: fetchFrIrveChargingDetail,
};
