...
const STATION_ID_PREFIX = "lt-vialietuva:";
...
const reader = createStaticPoiReader<EvChargingStation>({
  ...
  // [west, south, east, north] — Lithuania.
...
export async function searchLtVialietuvaCharging(bbox: BoundingBox): Promise<EvChargingStation[]> {
  return reader.search(getRuntimeContext(), toBboxTuple(bbox));
}

export async function fetchLtVialietuvaChargingDetail(
  itemId: string,
...
export const ltVialietuvaSource: EvChargingSource = {
  id: "lt-vialietuva",
  priority: getEvChargingSourcePriority("lt-vialietuva"),
  search: searchLtVialietuvaCharging,
  canFetchDetail: (itemId) => itemId.startsWith(STATION_ID_PREFIX),
  fetchDetail: fetchLtVialietuvaChargingDetail,
};
