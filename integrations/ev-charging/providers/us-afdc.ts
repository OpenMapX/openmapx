...
interface UsAfdcResponse {
  fuel_stations?: UsAfdcStation[];
  alt_fuel_station?: UsAfdcStation;
}

const US_AFDC_NEAREST_URL = "https://developer.nlr.gov/api/alt-fuel-stations/v1/nearest.json";
const US_AFDC_DETAIL_BASE = "https://developer.nlr.gov/api/alt-fuel-stations/v1";
const US_AFDC_STATION_URL = "https://afdc.energy.gov/stations/";
...
let usAfdcApiKey: string | undefined;

export function setUsAfdcApiKey(value: string | undefined): void {
  usAfdcApiKey = value && value.length > 0 ? value : undefined;
}
...
function buildConnectors(station: UsAfdcStation): EvChargingStation["connectors"] {
...
function stationToCanonical(station: UsAfdcStation): EvChargingStation | null {
...
function authHeaders(): Record<string, string> {
  ...
  return {
    "X-Api-Key": usAfdcApiKey as string,
  };
}

export async function searchUsAfdcCharging(bbox: BoundingBox): Promise<EvChargingStation[]> {
  if (!usAfdcApiKey || !bboxOverlaps(bbox, COVERAGE)) return [];
  ...
  const data = await fetchJson<UsAfdcResponse>(`${US_AFDC_NEAREST_URL}?${params.toString()}`, {
    headers: authHeaders(),
...
export async function fetchUsAfdcChargingDetail(itemId: string): Promise<EvChargingStation | null> {
  if (!usAfdcApiKey) return null;
  const id = itemId.startsWith("us-afdc:") ? itemId.slice("us-afdc:".length) : itemId;
  const data = await fetchJson<UsAfdcResponse>(
    `${US_AFDC_DETAIL_BASE}/${encodeURIComponent(id)}.json`,
    {
      headers: authHeaders(),
      errorMessage: ({ status }) => `AFDC API error: ${status}`,
    },
...
export const usAfdcSource: EvChargingSource = {
  id: "us-afdc",
  priority: getEvChargingSourcePriority("us-afdc"),
  search: searchUsAfdcCharging,
  canFetchDetail: (itemId) => itemId.startsWith("us-afdc:"),
  fetchDetail: fetchUsAfdcChargingDetail,
};
