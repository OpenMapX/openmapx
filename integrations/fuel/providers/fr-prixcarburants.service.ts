...
interface FrPrixcarburantsRecord {
  id: string;
  adresse?: string;
  ville?: string;
...
interface FrPrixcarburantsResponse {
  results: FrPrixcarburantsRecord[];
}

export class FrPrixcarburantsService implements FuelPriceProvider {
  readonly name = "fr-prixcarburants";
  ...
  async searchStations(bbox: BoundingBox): Promise<FuelStation[]> {
    ...
    const data = await fetchJson<FrPrixcarburantsResponse>(url.toString(), {
