...
async function fetchByFuelType(
  ...
  return fetchJson<AtEcontrolStation[]>(url.toString(), {
    errorMessage: ({ status }) => `Austria fuel API error: ${status}`,
  });
}

export class AtEcontrolService implements FuelPriceProvider {
  readonly name = "at-econtrol";
