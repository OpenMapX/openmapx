...
describe("SunTimeLayer time zones", () => {
  ...
  it("resets tzLoading instead of sticking on when the sub-toggle turns off before the fetch resolves", async () => {
    ...
    await vi.waitFor(() => expect(useSunTimeStore.getState().tzLoading).toBe(true));

    // Turning the sub-toggle off leaves the fetch itself in flight — only the
    // component's interest in its result is gone. A hand-rolled cancelled
    // flag would make the `finally` below skip the reset and leave the
    // legend's progress bar spinning forever.
    act(() => {
      useSunTimeStore.setState({ showTimeZones: false });
    });
    expect(useSunTimeStore.getState().tzLoading).toBe(true);

    await act(async () => {
      resolveFetch({ ok: true, json: async () => TZ_FIXTURE });
      await pending;
    });

    expect(useSunTimeStore.getState().tzLoading).toBe(false);
  });
});
