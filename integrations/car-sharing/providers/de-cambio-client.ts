...
const DE_CAMBIO_BASE = "https://cwapi.cambio-carsharing.com/pub";
const FETCH_TIMEOUT_MS = 8_000;

/** Cambio regions with approximate center coordinates for bbox matching. */
const DE_CAMBIO_REGIONS: {
  code: string;
  name: string;
  lat: number;
  lng: number;
...
function findRegionsInBbox(bbox: BoundingBox): typeof DE_CAMBIO_REGIONS {
  const padding = 0.5; // ~50km
  return DE_CAMBIO_REGIONS.filter(
    (r) =>
...
async function fetchRegionStations(
  region: (typeof DE_CAMBIO_REGIONS)[0],
  bbox: BoundingBox,
): Promise<SharedMobilityStation[]> {
  try {
    const stations = await fetchJson<DeCambioStation[]>(
      `${DE_CAMBIO_BASE}/${region.code}/stations`,
      {
...
export const deCambioClient: RegionalCarSharingClient = {
  id: "de-cambio",
  name: "Cambio",
  regions: DE_CAMBIO_REGIONS.map((r) => ({
