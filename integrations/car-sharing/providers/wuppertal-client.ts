...
interface WuppertalGeoJSON {
  type: "FeatureCollection";
  features: WuppertalFeature[];
}
...
function parse(body: string): SharedMobilityStation[] {
  const geojson = JSON.parse(body) as WuppertalGeoJSON;
  ...
  for (const feature of geojson.features) {
    ...
    stations.push({
      id: `wuppertal/${lng.toFixed(5)},${lat.toFixed(5)}`,
      name,
...
export const wuppertalClient = createStaticCarSharingClient({
  id: "wuppertal",
