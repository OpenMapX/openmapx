...
beforeEach(() => {
  sandbox = mkdtempSync(join(tmpdir(), "openmapx-dawarich-app-entrypoint-"));
});
...
function runHarness(secrets: Partial<Record<(typeof secretNames)[number], string>>) {
  ...
  const source = readFileSync(wrapperPath, "utf-8")
    ...
    .replace("/usr/local/bin/web-entrypoint.sh", stockEntrypoint);
  const harness = join(sandbox, "openmapx-entrypoint.sh");
  writeFileSync(harness, source, { mode: 0o700 });
  chmodSync(harness, 0o700);
  return spawnSync("/bin/sh", [harness, "bin/rails", "server"], { encoding: "utf-8" });
}

describe("Dawarich app secret-file entrypoint", () => {
  it("loads only the three declared secrets and execs the stock web entrypoint with original args", () => {
    ...
    expect(result.stdout).toBe("loaded:13:16:15 args:bin/rails server\n");
  ...
  it("fails closed on an empty secret without revealing its value", () => {
    const result = runHarness({
      DATABASE_PASSWORD: "db-secret-123",
      SECRET_KEY_BASE: "",
      OIDC_CLIENT_SECRET: "oidc-secret-789",
    ...
    expect(result.stderr).toContain("SECRET_KEY_BASE");
    expect(result.stdout + result.stderr).not.toContain("db-secret-123");
    expect(result.stdout + result.stderr).not.toContain("oidc-secret-789");
  });
  ...
  it("has valid POSIX shell syntax and a narrow static security boundary", () => {
    ...
    expect(source).toContain('exec /usr/local/bin/web-entrypoint.sh "$@"');
