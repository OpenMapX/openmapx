...
/**
 * Base URL the app-web container uses for server-side fetches against
 * app-api. Server components shouldn't go through Traefik because the public
 * domain may be gated behind basic auth, IP allowlists, or just incur an
 * extra TLS round-trip the user paid for in cookies — the API container is
 * always reachable directly on the internal Docker network.
 *
 * Resolution order:
