...
export class Sha256 {
  ...
  digestHex(): string {
    ...
    clone.bytesHashed = this.bytesHashed;

    const bitLength = clone.bytesHashed * 8;
    clone.update(Uint8Array.of(0x80));
    while (clone.blockLength !== 56) clone.update(Uint8Array.of(0));
    ...
    clone.update(lengthBytes);
    return Array.from(clone.state, (value) => value.toString(16).padStart(8, "0")).join("");
  }
  ...
  private transform(block: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const temp1 = (h + sigma1 + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0;
