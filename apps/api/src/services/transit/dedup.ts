 ...
 * Sorts tokens alphabetically so "Königlicher Hof, Moers" and
 * "Moers Königlicher Hof" produce the same key.
 */
...
export function deduplicateStops(stops: TransitStop[]): TransitStop[] {
  ...
  // Pairwise comparison — O(n²) but n is small (nearby stops query)
