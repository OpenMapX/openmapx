...
async function getSearchByTags() {
  const { wikimediaProvider } = await import("@integrations/photos-wikimedia/provider.js");
  const { searchByTags } = wikimediaProvider;
  if (!searchByTags) throw new Error("searchByTags not defined");
  return searchByTags.bind(wikimediaProvider);
}

describe("wikimediaProvider.searchByTags", () => {
  it("returns [] when no wikimedia_commons tag", async () => {
    const searchByTags = await getSearchByTags();
    const result = await searchByTags({ name: "Test" });
  ...
  it('no prefix → treated as category (prepends "Category:")', async () => {
    ...
    const searchByTags = await getSearchByTags();
    await searchByTags({ wikimedia_commons: "Berlin" });

    const url = mockFetch.mock.calls[0][0] as string;
    expect(url).toContain("gcmtitle=Category%3ABerlin");
  });

  it("returns [] when category has no parseable pages", async () => {
  ...
  it("limits to 6 photos per category (MAX_TAG_PHOTOS)", async () => {
  ...
  it("returns [] when fetch throws (timeout)", async () => {
