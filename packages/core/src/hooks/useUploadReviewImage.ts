...
/**
 * Uploads a review image via the backend proxy. The image is first re-encoded
 * client-side to strip EXIF/location/camera metadata before leaving the
 * browser. Returns the absolute URL on the Mangrove file CDN, ready to plug
 * into a review's `images[]`.
 */
export function useUploadReviewImage() {
  return useMutation({
    mutationFn: async (file: File): Promise<UploadResponse> => {
      ...
      return apiClient.post<UploadResponse>(API_ENDPOINTS.reviewImage, {
        dataUrl,
