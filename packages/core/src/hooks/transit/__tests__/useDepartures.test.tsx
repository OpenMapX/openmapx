...
describe("useDepartures", () => {
  ...
  it("fetches departures for a stop and forwards the minutes window", async () => {
    const envelope = {
      data: [{ tripId: "t1", line: "U6" }],
      attributions: [{ name: "MOTIS" }],
      freshness: undefined,
    };
    const spy = vi.spyOn(apiClient, "get").mockResolvedValue(envelope as never);

    const { result } = renderHook(() => useDepartures("ms:stop 1", 30), {
    ...
    expect(spy).toHaveBeenCalledWith("/api/integrations/transit/stops/ms%3Astop%201/departures", {
      minutes: "30",
    });
  });
  ...
  it("defaults the minutes window to 60", async () => {
    ...
    const { result } = renderHook(() => useDepartures("s1"), {
      wrapper: createQueryWrapper(),
    });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(spy).toHaveBeenCalledWith("/api/integrations/transit/stops/s1/departures", {
      minutes: "60",
    });
  });
  ...
  it("does not fire when the stop id is null", () => {
    ...
    const { result } = renderHook(() => useDepartures(null), {
      wrapper: createQueryWrapper(),
    });
